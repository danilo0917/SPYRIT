...
export default function StakingModal({ isOpen, onDismiss, stakingInfo, userLiquidityUnstaked }: StakingModalProps) {
  ...
  // pair contract for this token to be staked
    const dummyPair = (stakingInfo.name !== 'SPYRIT-USDC' &&  stakingInfo.name !== 'SPYRIT-DAI' && stakingInfo.name !== 'SPYRIT-ETHER' && stakingInfo.name !== 'SPYRIT-AVAX' && stakingInfo.name !==  'SPYRIT-FTM' ?
  ...
  async function onStake() {
    ...
    if (stakingContract && parsedAmount && deadline) {
      ...
      if (approval === ApprovalState.APPROVED) {
        ...
        await stakingContract.stake(`0x${parsedAmount.raw.toString(16)}`, { gasLimit: 500000 })
        ...
        <LoadingView onDismiss={wrappedOnDismiss}>
          <AutoColumn gap="12px" justify={'center'}>
            ...
            {!stakingInfo.isTokenOnly ?
              <TYPE.body fontSize={20}>{parsedAmount?.toSignificant(4)}Uniswap V2</TYPE.body>
              :
              <TYPE.body fontSize={20}>{parsedAmount?.toSignificant(4)} SPYRIT TOKEN</TYPE.body>
            }
          </AutoColumn>
        </LoadingView>
      )}
      {attempting && hash && (
        <SubmittedView onDismiss={wrappedOnDismiss} hash={hash}>
          <AutoColumn gap="12px" justify={'center'}>
            ...
            <TYPE.body fontSize={20}>Deposited {parsedAmount?.toSignificant(4)}Uniswap V2</TYPE.body>
          </AutoColumn>
        </SubmittedView>
      )}
    </Modal>
